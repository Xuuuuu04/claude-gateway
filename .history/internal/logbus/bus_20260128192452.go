	...
	if b.db != nil {
		go func() {
			...
			_, err := b.db.ExecContext(ctx,
				`INSERT INTO request_logs (request_id, pool_id, provider_id, credential_id, client_key, facade, req_model, upstream_model, status, latency_ms, error_msg)
				 VALUES (?, ?, (SELECT provider_id FROM credentials WHERE id=?), ?, ?, ?, ?, ?, ?, ?, ?)`,
				ev.RequestID, ev.PoolID, ev.CredentialID, ev.CredentialID, ev.ClientKey, ev.Facade, ev.RequestModel, ev.UpstreamModel, ev.Status, ev.LatencyMs, ev.Error)
			if err != nil {
