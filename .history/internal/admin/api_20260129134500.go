...
func (h *Handler) listLogs(w http.ResponseWriter, r *http.Request) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&l.ID, &poolID, &provID, &credID, &l.ClientKey, &srcIP, &ua, &isTest, &stream, &reqBytes, &respBytes, &inTok, &outTok, &l.Facade, &l.RequestModel, &upModel, &status, &latency, &errMsg, &ts); err != nil {
			writeJSON(w, http.StatusInternalServerError, map[string]any{"error": err.Error()})
			return
		}
