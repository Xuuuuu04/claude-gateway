...
func (h *Handler) listProviders(w http.ResponseWriter, r *http.Request) {
	rows, err := h.db.QueryContext(r.Context(), `SELECT id, type, base_url, default_headers_json, model_map_json, enabled FROM providers ORDER BY id DESC`)
	if err != nil {
	...
	for rows.Next() {
		...
		var hdrs []byte
		var mm []byte
		if err := rows.Scan(&p.ID, &p.Type, &p.BaseURL, &hdrs, &mm, &p.Enabled); err != nil {
			writeJSON(w, http.StatusInternalServerError, map[string]any{"error": err.Error()})
			return
		}
...
func (h *Handler) createProvider(w http.ResponseWriter, r *http.Request) {
	...
	res, err := h.db.ExecContext(r.Context(), `INSERT INTO providers(type, base_url, default_headers_json, model_map_json, enabled) VALUES (?,?,?,?,?)`,
		in.Type, in.BaseURL, in.DefaultHeadersRaw, in.ModelMapRaw, in.Enabled)
	if err != nil {
		writeJSON(w, http.StatusBadRequest, map[string]any{"error": err.Error()})
		return
	}
...
func (h *Handler) updateProvider(w http.ResponseWriter, r *http.Request) {
	...
	_, err = h.db.ExecContext(r.Context(), `UPDATE providers SET type=?, base_url=?, default_headers_json=?, model_map_json=?, enabled=? WHERE id=?`,
		in.Type, in.BaseURL, in.DefaultHeadersRaw, in.ModelMapRaw, in.Enabled, id)
	if err != nil {
		writeJSON(w, http.StatusBadRequest, map[string]any{"error": err.Error()})
		return
	}
...
type credentialBulkItemDTO struct {
	Name             string `json:"name"`
	APIKey           string `json:"api_key,omitempty"`
	Weight           int    `json:"weight"`
	ConcurrencyLimit *int   `json:"concurrency_limit,omitempty"`
...
type credentialBulkDTO struct {
	ProviderID uint64                  `json:"provider_id"`
	Items      []credentialBulkItemDTO `json:"items"`
}

func (h *Handler) createCredentialsBulk(w http.ResponseWriter, r *http.Request) {
	var in credentialBulkDTO
	if err := readJSON(r, &in); err != nil {
		writeJSON(w, http.StatusBadRequest, map[string]any{"error": err.Error()})
		return
	}
	...
	out := make([]credentialDTO, 0, len(in.Items))
	for _, it := range in.Items {
		if strings.TrimSpace(it.Name) == "" || strings.TrimSpace(it.APIKey) == "" {
			writeJSON(w, http.StatusBadRequest, map[string]any{"error": "each item requires name and api_key"})
			return
		}
		blob, err := h.cipher.Encrypt([]byte(it.APIKey))
		if err != nil {
			writeJSON(w, http.StatusInternalServerError, map[string]any{"error": err.Error()})
			return
		}
		...
		res, err := stmt.ExecContext(r.Context(), in.ProviderID, it.Name, blob, last4v, weight, conc, it.Enabled)
		if err != nil {
			writeJSON(w, http.StatusBadRequest, map[string]any{"error": err.Error()})
			return
		}
		...
		out = append(out, credentialDTO{
			...
			KeyLast4:         last4v,
			Weight:           weight,
			ConcurrencyLimit: it.ConcurrencyLimit,
			Enabled:          it.Enabled,
		})
	}
