	...
	for attempt := 0; attempt < maxAttempts; attempt++ {
		...
		case "openai":
			...
			if !ok {
				...
				publish(up, status, time.Since(start), "upstream_unavailable", 0, 0, 0)
			...
			if req.Stream {
				...
				var respBytes int
				respBytes, inTok, outTok, err = copyOpenAISSEWithUsage(w, resp.Body)
...
func copyOpenAISSEWithUsage(w http.ResponseWriter, r io.Reader) (int, int64, int64, error) {
