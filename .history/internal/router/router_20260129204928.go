...
type ErrNoAvailableUpstream struct {
	Reason      string
	PoolID      uint64
	PoolName    string
	TierName    string
...
func (e *ErrNoAvailableUpstream) Error() string {
	msg := fmt.Sprintf("no available upstream for model '%s'", e.Model)
	if e.PoolName != "" {
		msg += fmt.Sprintf(" in pool '%s'", e.PoolName)
	}
	if e.TierName != "" {
		msg += fmt.Sprintf(" (tier: %s)", e.TierName)
	}
	if e.ProviderTag != "" {
		msg += fmt.Sprintf(" from provider %s", e.ProviderTag)
	}
	if e.Reason != "" {
		msg += ": " + e.Reason
	}
...
func (r *Router) GetPoolModels(ctx context.Context, clientKey string) ([]string, error) {
	...
	// 1. Add all keys from pool's model map (explicitly exposed at pool level)
	...
	// 2. Process Tiers (Primary)
	if len(pool.Tiers) > 0 {
		for _, tier := range pool.Tiers {
			if len(tier.Models) > 0 {
				// Tier is restricted to specific models
				...
				// Tier is "allow all", add models from its providers
				for _, it := range tier.Items {
					if prov, ok := cfg.providers[it.ProviderID]; ok {
						if len(prov.ModelMap) > 0 {
		...
		// 3. Process Legacy CredentialIDs (only if NO tiers)
		for _, cid := range pool.CredentialIDs {
			if cred, ok := cfg.credentials[cid]; ok {
				if prov, ok := cfg.providers[cred.ProviderID]; ok {
					if len(prov.ModelMap) > 0 {
						for k := range prov.ModelMap {
							if k != "" {
								modelSet[k] = true
...
func (r *Router) EndRequest(credentialID uint64, ok bool, status int, latency time.Duration) {
	...
	case http.StatusTooManyRequests:
		d := time.Duration(2*(1<<minInt(st.failures, 6))) * time.Second
		if d > 2*time.Minute {
			d = 2 * time.Minute
		}
	...
	default:
		if status >= 500 || status == 0 {
			...
			st.openUntil = now.Add(10 * time.Second)
		}
	}
}
...
func (r *Router) pickUpstream(ctx context.Context, clientKey, facade, model string, exclude map[uint64]bool) (RoutedUpstream, error) {
	...
	credID, err := r.pickCredentialFromPool(cfg, pool, facade, model, exclude)
	if err != nil {
		return RoutedUpstream{}, err
	}
...
func (r *Router) pickCredentialFromPool(cfg loadedConfig, pool poolRow, facade, model string, exclude map[uint64]bool) (uint64, error) {
	now := time.Now()

	// Track why credentials were skipped
	reasons := make(map[string]int)

	isAvailable := func(credID uint64) (bool, string) {
		...
		if r.isCredentialOpen(credID, now) {
			return false, "rate_limited_or_error_cooldown"
		}
	...
	if ent, ok := r.routeCache[key]; ok {
		if now.Before(ent.expiresAt) {
			if ok, _ := isAvailable(ent.credentialID); ok {
				r.routeCacheMu.RUnlock()
				return ent.credentialID, nil
			}
		}
		if !now.Before(ent.expiresAt) {
			r.routeCacheMu.RUnlock()
	...
	if len(pool.Tiers) > 0 {
		for _, tier := range pool.Tiers {
			...
			tierReasons := make(map[string]int)
			...
			pickBestKey := func(provID uint64) (uint64, float64, bool) {
				...
				for _, cid := range creds {
					if ok, reason := isAvailable(cid); !ok {
						tierReasons[reason]++
						continue
					}
			...
			if tier.Strategy == "priority" {
				...
				if len(candidates) > 0 {
					...
					return candidates[len(candidates)-1].cid, nil
				}
			}

			// If this tier applied to the model but found no available key, return error with tier info
			if len(tierReasons) > 0 {
				var rList []string
				for k, v := range tierReasons {
					rList = append(rList, fmt.Sprintf("%s(%d)", k, v))
				}
				return 0, &ErrNoAvailableUpstream{
					Reason:   strings.Join(rList, ", "),
					PoolID:   pool.ID,
					PoolName: pool.Name,
					TierName: tier.Name,
					Model:    model,
		...
		return 0, &ErrNoAvailableUpstream{
			Reason:   "no tiers matched this model or all providers in matched tiers failed",
			PoolID:   pool.ID,
			PoolName: pool.Name,
			Model:    model,
		}
	...
	if len(ids) == 0 {
		return 0, &ErrNoAvailableUpstream{
			Reason:   "pool has no credentials and no tiers",
			PoolID:   pool.ID,
			PoolName: pool.Name,
			Model:    model,
		}
