...
func (r *Router) pickUpstream(ctx context.Context, clientKey, facade, model string, exclude map[uint64]bool) (RoutedUpstream, error) {
	...
	credID, ok := r.pickCredentialFromPool(cfg, pool, exclude)
	if !ok {
		return RoutedUpstream{}, ErrNotConfigured
	}
...
func (r *Router) pickCredentialFromPool(cfg loadedConfig, pool poolRow, exclude map[uint64]bool) (uint64, bool) {
	now := time.Now()
...
func loadPools(ctx context.Context, db *sql.DB, out map[uint64]poolRow, byKey map[string]poolRow) error {
	rows, err := db.QueryContext(ctx, `SELECT id, name, client_key, strategy, credential_ids_json, model_map_json, enabled FROM pools WHERE enabled = 1`)
	if err != nil {
		return err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&id, &name, &ckey, &strategy, &idsJSON, &mmJSON, &enabled); err != nil {
			return err
		}
		var ids []uint64
