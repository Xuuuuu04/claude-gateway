...
func (h *Handler) listLogs(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := h.db.QueryContext(r.Context(),
		`SELECT id, pool_id, provider_id, credential_id, client_key, src_ip, user_agent, is_test, stream, request_bytes, response_bytes, input_tokens, output_tokens, facade, req_model, upstream_model, status, latency_ms, error_msg, ts 
	...
	for rows.Next() {
		var l logEntry
		if err := rows.Scan(&l.ID, &l.PoolID, &l.ProviderID, &l.CredentialID, &l.ClientKey, &l.SrcIP, &l.UserAgent, &l.IsTest, &l.Stream, &l.RequestBytes, &l.ResponseBytes, &l.InputTokens, &l.OutputTokens, &l.Facade, &l.RequestModel, &l.UpstreamModel, &l.Status, &l.LatencyMs, &l.Error, &l.CreatedAt); err != nil {
			writeJSON(w, http.StatusInternalServerError, map[string]any{"error": err.Error()})
			return
		}
		out = append(out, l)
...
func (h *Handler) getStats(w http.ResponseWriter, r *http.Request) {
	...
	for dailyRows.Next() {
		var s dayStat
		if err := dailyRows.Scan(&s.Day, &s.Total, &s.Success, &s.InputTokens, &s.OutputTokens, &s.AvgLatencyMs); err != nil {
			continue
		}
		days = append(days, s)
	}
	...
	err = h.db.QueryRowContext(r.Context(),
		`SELECT 
		 ...
		 FROM request_logs 
		 WHERE DATE(ts) = DATE(NOW())`).Scan(&today.Total, &today.Success, &today.InputTokens, &today.OutputTokens, &today.AvgLatencyMs)
	if err != nil {
		// ignore
	}
