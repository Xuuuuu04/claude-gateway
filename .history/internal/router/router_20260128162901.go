...
func loadCredentials(ctx context.Context, db *sql.DB, out map[uint64]credentialRow) error {
	rows, err := db.QueryContext(ctx, `SELECT id, provider_id, api_key_ciphertext, weight, enabled FROM credentials WHERE enabled = 1`)
	if err != nil {
		return err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&id, &providerID, &blob, &weight, &enabled); err != nil {
			return err
		}
		if weight <= 0 {
			weight = 1
