...
func (h *Handler) responses(w http.ResponseWriter, r *http.Request) {
	...
	if up.ProviderType != "openai" {
		if up.ProviderType == "anthropic" {
			...
			if strings.TrimSpace(req.Instructions) != "" {
				msgs = append(msgs, map[string]any{"role": "system", "content": strings.TrimSpace(req.Instructions)})
			}

			switch v := req.Input.(type) {
			case nil:
				writeError(w, http.StatusBadRequest, "invalid_request_error", "missing_input", "input is required")
				return
			case string:
			...
			case []any:
				for _, it := range v {
					...
					if !ok {
						writeError(w, http.StatusBadRequest, "invalid_request_error", "invalid_input", "input items must be objects")
						return
					}
					...
					if role == "" {
						writeError(w, http.StatusBadRequest, "invalid_request_error", "invalid_input", "input items must include role")
						return
					}
					...
					case []any:
						...
						for _, pi := range c {
							...
							if !ok {
								writeError(w, http.StatusBadRequest, "invalid_request_error", "invalid_input", "content parts must be objects")
								return
							}
							pt, _ := pm["type"].(string)
							switch pt {
							...
							case "input_image":
								...
								if u == "" {
									writeError(w, http.StatusBadRequest, "invalid_request_error", "invalid_input", "input_image.image_url is required")
									return
								}
								if !(strings.HasPrefix(u, "https://") || strings.HasPrefix(u, "data:image/")) {
									writeError(w, http.StatusBadRequest, "invalid_request_error", "invalid_input", "input_image.image_url must be https:// or data:image/*")
									return
								}
								parts = append(parts, map[string]any{"type": "image_url", "image_url": map[string]any{"url": u}})
							default:
								writeError(w, http.StatusBadRequest, "invalid_request_error", "invalid_input", "unsupported content part type")
								return
							}
						}
						msgs = append(msgs, map[string]any{"role": role, "content": parts})
					default:
						writeError(w, http.StatusBadRequest, "invalid_request_error", "invalid_input", "unsupported content type")
						return
					}

					if role == "tool" {
						if _, ok := m["tool_call_id"].(string); !ok {
							writeError(w, http.StatusBadRequest, "invalid_request_error", "invalid_input", "tool messages require tool_call_id")
							return
						}
					}
					if tc := m["tool_call_id"]; tc != nil {
						msgs[len(msgs)-1].(map[string]any)["tool_call_id"] = tc
					}
					if tcs := m["tool_calls"]; tcs != nil {
						msgs[len(msgs)-1].(map[string]any)["tool_calls"] = tcs
					}
				}
			default:
				writeError(w, http.StatusBadRequest, "invalid_request_error", "invalid_input", "input must be string or array")
				return
			}
	...
	if req.Stream {
		err = openai.CopySSE(w, resp.Body)
		h.rtr.EndRequest(up.CredentialID, err == nil && resp.StatusCode < 500 && resp.StatusCode != http.StatusTooManyRequests, resp.StatusCode, time.Since(start))
		return
	}
	_, _ = io.Copy(w, resp.Body)
	h.rtr.EndRequest(up.CredentialID, resp.StatusCode < 500 && resp.StatusCode != http.StatusTooManyRequests, resp.StatusCode, time.Since(start))
}
...
func (h *Handler) listModels(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	clientKey, _ := ctx.Value(canonical.ContextKeyClientKey).(string)
	up, err := h.rtr.PickUpstream(ctx, clientKey, string(canonical.FacadeOpenAI), "")
	if err != nil {
		writeError(w, http.StatusBadGateway, "server_error", "routing_failed", "routing failed")
		return
	}
	if up.ProviderType != "openai" {
		writeError(w, http.StatusNotImplemented, "server_error", "not_implemented", "models listing not supported for this upstream")
		return
	}
	resp, err := openai.DoModels(ctx, openai.Upstream{
		BaseURL: up.BaseURL,
		APIKey:  string(up.APIKey),
		Headers: up.Headers,
	})
	...
	w.WriteHeader(resp.StatusCode)
	_, _ = io.Copy(w, resp.Body)
	h.m.ObserveRequest(string(canonical.FacadeOpenAI), up.ProviderType, resp.StatusCode, 0*time.Millisecond)
}
