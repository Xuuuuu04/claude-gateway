...
func (h *Handler) chatCompletions(w http.ResponseWriter, r *http.Request) {
	...
	for attempt := 0; attempt < maxAttempts; attempt++ {
		...
		if attempt == 0 {
			...
			up, err = h.rtr.PickUpstreamExclude(ctx, string(canonical.FacadeOpenAI), req.Model, exclude)
		}
		if err != nil {
		...
		case "anthropic":
			...
			if status < 200 || status >= 300 {
				...
				writeError(w, http.StatusBadGateway, "server_error", "upstream_failed", "upstream error")
				return
			}
			if req.Stream {
